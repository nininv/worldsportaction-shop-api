...
export interface UpdateCartQueryParams {
  ...
  securePaymentOptions: [{ securePaymentOptionRefId: number }];
...
export class CartController extends BaseController {
  ...
  async updateCartProducts(
    ...
    try {
      const updatedCartProducts = await this.shopService.updateCartProducts(
        shopUniqueKey,
        cartProducts,
      );
  ...
  async getShopProducts(
    ...
    try {
      ...
      return res
        .status(500)
        .send('Something went wrong. Please contact administrator' + err.message);
    }
  }

  @Authorized()
  @Get('/organisations')
  async getShopOrganisations(@HeaderParam('authorization') user: User, @Res() res: Response) {
    try {
  ...
  async paymentSubmit(
    ...
    try {
      ...
      if (PAYMENT_STATUS === 'success') {
        return res
          .status(200)
          .send({ success: true, message: 'no need to pay, as its already paid' });
      } else if (PAYMENT_STATUS === 'pending') {
        return res
          .status(212)
          .send({ success: true, message: 'no need to pay, as its already paid' });
      } else {
        const account = await this.organisationService.checkOrganisationStripeAccount(
          result.cartProducts,
        );
        ...
        if (!account.stripeAccountIdError) {
          ...
          if (requestBody.paymentType === AppConstants.card) {
            if (paymentIntent.status === 'succeeded') {
              ...
              await this.performShopPayment(
                paymentIntent,
                result.cart,
                requestBody.payload.cartProducts,
                INVOICE_ID,
                requestBody.paymentType,
              );
            } else {
              return res
                .status(212)
                .send({ success: false, message: 'cannot create payment, an error occured' });
            }
          }
        } else {
      ...
      if (err.type === 'StripeCardError') {
        ...
        switch (err.code) {
          case 'card_declined':
            switch (err.decline_code) {
              case 'insufficient_funds':
                return res
                  .status(400)
                  .json({ success: false, message: AppConstants.cardDeclinedInsufficientFunds });
              case 'lost_card':
                return res
                  .status(400)
                  .json({ success: false, message: AppConstants.cardDeclinedLostCard });
              case 'stolen_card':
                return res
                  .status(400)
                  .json({ success: false, message: AppConstants.cardDeclinedStolenCard });
              case 'generic_decline':
                return res
                  .status(400)
                  .json({ success: false, message: AppConstants.cardGenericDecline });
              default:
          ...
          case 'processing_error':
            return res
              .status(400)
              .json({ success: false, message: AppConstants.cardProcessingError });
          case 'incorrect_number': // unlikely, since this will not pass the frontend
            return res
              .status(400)
              .json({ success: false, message: AppConstants.cardIncorrectNumber });
          default:
  ...
  async performShopPayment(paymentIntent, cart, cartProducts, INVOICE_ID, paymentType) {
    try {
      ...
      if (isArrayPopulated(cartProducts)) {
        ...
        for (let prod of cartProducts) {
          ...
          let mOrgData = await this.organisationService.findOrganisationByUniqueKey(
            prod.organisationId,
          );
        ...
        if (!accountError) {
          ...
          let orderGrpObj = await this.orderGroupService.getOrderGroupObj(
            cart.createdBy,
            shopTotalFee,
          );
          ...
          for (let prod of cartProducts) {
            ...
            if (paymentType != AppConstants.directDebit) {
              transferForShopFee = await this.shopStripePayment(
                prod,
                paymentIntent,
            ...
            const organisation = await this.organisationService.findOrganisationByUniqueKey(
              prod.organisationId,
            );
            const orderObj = await this.orderService.getOrderObj(
              paymentMethod,
              paymentStatus,
              cart,
              prod,
            ...
            let sellProductObj = await this.sellProductService.getSellProductObj(
              cart,
              order,
              prod,
              sku,
            ...
            if (prod.inventoryTracking) {
              ...
              await this.skuService.updateQuantity(
                prod.skuId,
  ...
  async getStripePaymentIntent(
    ...
    try {
      ...
      if (totalStripeFee > 0) {
        if (
          isNotNullAndUndefined(paymentBody.token) &&
          isNotNullAndUndefined(paymentBody.token.id) &&
          ...
          if (isNullOrEmpty(stripeCustomerId)) {
            ...
            if ((cards && cards.data && cards.data.length) || cards.data.length == 0) {
              await this.updateStripeCustorAccountDefaultSource(
                stripeCustomerId,
                paymentBody.token,
              );
            }
      ...
      const updateTransaction = await this.invoiceService.updateErrorMessageAndPaymentTypeWithTransaction(
        cart.id,
        paymentType,
        null,
        PAYMENT_INTENT_ID,
  ...
  private async updateStripeCustorAccountDefaultSource(stripeCustomerAccountId, token) {
    try {
      logger.debug(
        `UpdateStripeCustorAccountDefaultSource Card Id  CustId ${stripeCustomerAccountId}`,
      );
      const card = await stripe.customers.createSource(stripeCustomerAccountId, {
        source: token.id,
      });
      console.log('card ' + JSON.stringify(card.id));
      const customer = await stripe.customers.update(stripeCustomerAccountId, {
