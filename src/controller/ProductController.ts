...
export class ProductController extends BaseController {
  ...
  async post(
    ...
    try {
      ...
      const product = await this.productService.createOrUpdateProduct(
        paramObj,
        productPhoto,
        currentUser.id,
      );
  ...
  async getProduct(
    ...
    try {
      ...
      const found = await this.productService.getProductList(
        search,
        sort,
        pagination,
        organisationId,
  ...
  async getProductList(
    ...
    try {
      ...
      for (const key in organisationUniqueKeys) {
        const organisation = await this.organisationService.findByUniquekey(
          organisationUniqueKeys[key],
        );
        organisationIds = [...organisationIds, organisation];
      }
      const listObject = await this.productService.getProductListForEndUser(
        type,
        organisationIds,
        pagination,
      );
      if (listObject) {
        let responseObject = paginationData(
          stringTONumber(listObject.count),
          pagination.limit,
          pagination.offset,
        );
  ...
  @Delete('/image')
  async deleteImageRouter(@QueryParam('url') url: string, @Res() response: Response) {
    try {
  ...
  @Authorized()
  @Get('')
  async getProductById(@QueryParam('id') id: number, @Res() response: Response) {
    try {
