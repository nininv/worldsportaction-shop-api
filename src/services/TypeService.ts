...
export default class TypeService extends BaseService<Type> {
  ...
  public async getList(organisationId): Promise<Type[]> {
    try {
      const organisationFirstLevel = organisationId
        ? await this.getAffiliatiesOrganisations([organisationId], 3)
        : [];
      const organisationSecondLevel = organisationId
        ? await this.getAffiliatiesOrganisations([organisationId], 4)
        : [];
  ...
  public async saveOrUpdateTypeList(
    types,
    userId: number,
    organisationId: number,
  ): Promise<Type[]> {
