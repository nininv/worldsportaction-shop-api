...
export default class InvoiceService extends BaseService<Invoice> {
  ...
  public async findByCartId(cartId: number): Promise<Invoice> {
    return await this.entityManager
      ...
      .andWhere('inv.isDeleted = 0')
      .execute();
  }

  public async getInvoiceReciptId(): Promise<any> {
    let query = await this.entityManager.query(
      `select IFNULL(receiptId, 100000) as receiptId from wsa_registrations.invoice order by id desc LIMIT 1`,
    );
  ...
  public async updateErrorMessageAndPaymentTypeWithTransaction(
    cartId: number,
    paymentType: string,
    error: string,
    ...
    try {
      ...
      if (isArrayPopulated(getInvoiceData)) {
        ...
        i.id = INVOICE_ID;
        i.cartId = cartId;
        if (paymentType) i.paymentType = paymentType;

        i.stripeSourceTransaction = isNotNullAndUndefined(stripeSourceTransaction)
