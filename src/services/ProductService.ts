...
export default class ProductService extends BaseService<Product> {
  ...
  public async getCount(condition: string, variables): Promise<number> {
    try {
      const count = await getConnection()
        ...
        .leftJoinAndSelect('product.SKU', 'SKU', 'SKU.isDeleted = 0  AND SKU.quantity > :min', {
          min: 0,
        })
        .leftJoinAndSelect(
          'SKU.productVariantOption',
          'productVariantOption',
          'productVariantOption.isDeleted = 0',
        )
        .leftJoinAndSelect(
          'productVariantOption.variant',
          'productVariant',
          'productVariant.isDeleted = 0',
        )
  ...
  public async getMany(
    ...
    try {
      const list = await getConnection()
        ...
        .where(condition, variables)
        .orderBy(
          sort && sort.sortBy ? `product.${sort.sortBy}` : null,
  ...
  public async getOne(condition: string, variables): Promise<Product> {
    try {
      const product = await getConnection()
        ...
        .leftJoinAndSelect('product.images', 'images')
        .leftJoinAndSelect('product.type', 'type')
        .leftJoinAndSelect('product.SKU', 'SKU', 'SKU.productId = :id AND SKU.isDeleted = 0', {
          id: variables.id,
        })
        .leftJoinAndSelect(
          'SKU.productVariantOption',
  ...
  public async getProductListForEndUser(
    type: number,
    organisationIds: number[],
    paginationData,
  ): Promise<{ count: number; result: ProductForEndUser[] }> {
    try {
      ...
      const products = await this.getMany(
        condition,
        { type, organisationIds, organisationFirstLevel, organisationSecondLevel },
        paginationData,
      );
      let resProduct = [];
      ...
      const count = await this.getCount(condition, {
        type,
        organisationIds,
  ...
  public async getAffiliatiesOrganisations(
    organisationId: number[],
    level: number,
  ): Promise<number[]> {
    try {
      ...
      for (const key in organisationId) {
        ...
        if (affiliatesOrganisations) {
          organisations = [
            ...organisations,
            ...affiliatesOrganisations.map(org => org.inputOrganisationId),
          ];
        }
      }
  ...
  public async getOrganisationLogo(createByOrg): Promise<ImageLogo> {
    try {
      ...
      if (logo) {
        const { createdBy, id, isDeleted, updatedBy, updatedOn, logoUrl, organisationId } = logo;
  ...
  public async addProduct(data, images: Image[], userId: number): Promise<ParseProduct> {
    ...
    try {
      ...
      const variantService = new ProductVariantService();
      const savedVariants = await variantService.saveProductVariantsAndOptions(
        variants,
        product.id,
        userId,
      );
  ...
  public async restoreProduct(id: number, userId): Promise<ParseProduct> {
    try {
      const a = await this.entityManager
        .createQueryBuilder(Product, 'product')
        .update(Product)
  ...
  public async createOrUpdateProduct(
    data,
    productPhotos: Express.Multer.File[],
    userId,
  ): Promise<ParseProduct> {
    const typeService = new TypeService();
    try {
      if (data.id) {
        const {
        ...
        const deletingImage = parseProduct.images.filter(
          image => !data.images || data.images.findIndex(img => img.id === image.id) === -1,
        );
        let images = [];
        if (productPhotos) {
  ...
  public async getShopProducts(requestBody: GetProductQueryParams) {
    try {
      ...
      if (requestBody.organisationUniqueKey != '-1') {
        let organisation = await this.findOrganisationByUniquekey(
        ...
        if (isArrayPopulated(organisationIds)) {
          const organisationFirstLevel = await this.getAffiliatiesOrganisations(organisationIds, 3);
          const organisationSecondLevel = await this.getAffiliatiesOrganisations(
            organisationIds,
            4,
          );
          ...
          result = await this.entityManager.query(
            'call wsa_shop.usp_registration_products(?,?,?,?,?,?)',
            [
              organisationIdList,
              organisationFirstLevelList,
              organisationSecondLevelList,
              requestBody.typeId,
              limit,
              offset,
            ],
        ...
        result = await this.entityManager.query(
          'call wsa_shop.usp_registration_shop_products(?,?,?,?)',
      ...
      if (userRegId == null) {
        query = await this.entityManager.query(
          `select DISTINCT  o.id as organisationId from wsa_users.organisation o
                    ...
                    where r.registrationUniqueKey = ? and o.isDeleted = 0 `,
  ...
  public async findOrganisationByUniquekey(organisationUniqueKey) {
    try {
      let query = await this.entityManager.query(
        `select * from wsa_users.organisation o where organisationUniqueKey = ?`,
        [organisationUniqueKey],
      );
