...
export default class SKUService extends BaseService<SKU> {
  ...
  public async saveSKU(price, cost, skuCode, barcode, quantity, productId, userId) {
    ...
    await this.addToRelation<SKU>({ model: 'Product', property: 'SKU' }, productId, savedSKU);
    return savedSKU;
  }
    ...
    try {
      ...
      if (!sku) {
        ...
        await this.entityManager
          ...
          .execute();
        await this.entityManager
          .createQueryBuilder(ProductVariantOption, 'productVariantOption')
  ...
  public async restoreProductVariants(id: number, userId): Promise<void> {
    try {
      const sku = await getConnection()
        .getRepository(SKU)
        .createQueryBuilder('SKU')
