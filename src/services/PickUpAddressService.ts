...
export default class PickUpAddressService extends BaseService<Type> {
  ...
  public async updateAddress(
    ...
    try {
      const { address, suburb, postcode, state, id, pickupInstruction } = data;
      const updatedAddress = await this.entityManager
        .createQueryBuilder(PickUpAddress, 'pickUpAddress')
  ...
  public async saveOrUpdateAddress(
    addressObj,
    userId: number,
    organisationId: number,
    organisationUniqueKey: string,
    ...
    try {
      ...
      if (addressObj.id && addressObj.id > 0) {
        ...
        if (
          addressFromBD.address !== addressObj.address ||
          ...
          newAddress = await this.updateAddress(
            addressObj,
            organisationId,
            organisationUniqueKey,
            userId,
        ...
        newAddress = await this.saveAdress(
          addressObj,
          organisationId,
          organisationUniqueKey,
          userId,
